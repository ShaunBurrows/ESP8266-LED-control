...
const char *password = "//password for network";   // The password required to connect to it, leave blank for an open network
...
#define LED_R1   16           // specify the pins with an RGB LED connected (R1, G1, B1 first strip etc.)
#define LED_G1   5
#define LED_B1   4
...
void setup() {
  pinMode(LED_R1, OUTPUT);    // the pins with LEDs connected are outputs
...
// start warm white (40W tungsten, 40% brightness (100W/40% = 100,84,67)
  analogWrite(LED_R1, 100);
  analogWrite(LED_G1, 77);
  analogWrite(LED_B1, 56);

  analogWrite(LED_R2, 100);
  analogWrite(LED_G2, 77);
  analogWrite(LED_B2, 56);

  analogWrite(LED_R3, 100);
  analogWrite(LED_G3, 77);
  analogWrite(LED_B3, 56);
...
void startWiFi() { // Start a Wi-Fi access point, and try to connect to some given access points.
  ...
  wifiMulti.addAP("//ssid", "// your password");   // add Wi-Fi networks you want to connect to
...
void startOTA() { // Start the OTA service
  ...
  ArduinoOTA.onStart([]() {
    ...
    digitalWrite(LED_R1, 0);    // turn off the LEDs
    digitalWrite(LED_G1, 0);
    digitalWrite(LED_B1, 0);
    digitalWrite(LED_R2, 0);    // turn off the LEDs
    digitalWrite(LED_G2, 0);
    digitalWrite(LED_B2, 0);
    digitalWrite(LED_R3, 0);    // turn off the LEDs
    digitalWrite(LED_G3, 0);
    digitalWrite(LED_B3, 0);
  });
  ArduinoOTA.onEnd([]() {
    Serial.println("\r\nEnd");
  });
  ...
  switch (type) {
    ...
    case WStype_TEXT:                     // if new text data is received
      ...
      if (payload[0] == '#') {            // we get RGB data
        int S3 = payload[1];                             // first char to determine if strip 3 will be changed
        int S2 = payload[2];
        int S1 = payload[3];
        uint32_t rgb = (uint32_t) strtol((const char *) &payload[4], NULL, 16);   // decode rgb data
        int r = ((rgb >> 20) & 0x3FF);                     // 10 bits per color, so R: bits 20-29
        int g = ((rgb >> 10) & 0x3FF);                     // G: bits 10-19, & 0x3FF adds leading zeros
        int b =          rgb & 0x3FF;                      // B: bits  0-9

        if (S1 == '1') {                                      // write first strip
          analogWrite(LED_R1, r);                        // write it to the LED output pins
          analogWrite(LED_G1, g);
          analogWrite(LED_B1, b);  }

        if (S2 == '1') {                                      // write second strip
          analogWrite(LED_R2, r);                        // write it to the LED output pins
...
//          analogWrite(LED_B3, b);  }
//
//        if (S4 == 1) {                                      // write fourth strip
//          analogWrite(LED_R4, r);                        // write it to the LED output pins
//          analogWrite(LED_G4, g);
//          analogWrite(LED_B4, b);  }
